...
# reenviar o anúncio ao Publishing Lab. Não inventa dados.
# ==========================================================

def s27_compact_title(value, limit=60):
...
def s27_known_description(context):
    product = context.get("product") or {}
    rows = []
    ...
    if name:
        rows.append(name)
    if brand:
        rows.append(f"Marca: {brand}")
    for attr in context.get("attributes") or []:
    ...
    rows.extend([
        "",
        "Produto novo. Confira compatibilidade, medidas e especificações antes da compra.",
        "Conteúdo da embalagem e garantia conforme informações cadastradas no anúncio.",
    ])
    return "\n".join(rows)


async def s27_discover_category(title):
    query = str(title or "").strip()
    if not query:
        return {"success": False, "error": "Título ausente para descobrir categoria."}
    result = await ml_request(
        "/sites/MLB/domain_discovery/search?q=" + quote(query, safe="")
    )
    if not result.get("success"):
        return {"success": False, "error": "A descoberta de categoria não respondeu.", "result": result}
    data = result.get("data") or []
    if isinstance(data, dict):
        data = [data]
    first = data[0] if data else {}
    category_id = first.get("category_id") or first.get("category")
    if not category_id:
        return {"success": False, "error": "Nenhuma categoria foi encontrada.", "candidates": data[:5]}
    return {
        "success": True,
...
async def s27_remove_non_official_attributes(product_id, category_id):
    official = {
        str(row.get("attribute_id") or "").upper()
        for row in await s21_category_attrs(category_id)
    }
    current = await store.select(
        "product_marketplace_attributes",
        "select=id,attribute_id&product_id=eq." + quote(str(product_id), safe="-")
        + "&marketplace=eq.mercado_livre&category_id=eq." + quote(str(category_id), safe="-_")
    )
    removed = []
    for row in current.get("data") or []:
        aid = str(row.get("attribute_id") or "").upper()
        if aid and aid not in official:
            await store.delete(
                "product_marketplace_attributes",
                "id=eq." + quote(str(row.get("id")), safe="-")
            )
            removed.append(aid)
    return removed


async def s27_auto_correct_listing(listing_id):
    listing = await s19_get_listing(listing_id)
    if not listing:
        return {"success": False, "status_code": 404, "error": "Anúncio não encontrado."}
    context = await s19_product_context(listing.get("product_id"))
    ...
    fixed_title = s27_compact_title(original_title)
    if fixed_title and fixed_title != str(listing.get("title") or ""):
        await store.update(
            "listings",
            "id=eq." + quote(str(listing_id), safe="-"),
            {"title": fixed_title}
        )
        listing["title"] = fixed_title
        fixed.append({"code": "title_normalized", "value": fixed_title})

    # Categoria: usa exclusivamente a descoberta oficial do Mercado Livre.
    category_id = str(listing.get("category_id") or "").strip()
    if not category_id:
        discovered = await s27_discover_category(fixed_title or original_title)
        if discovered.get("success"):
            category_id = discovered.get("category_id")
            await store.update(
                "listings",
                "id=eq." + quote(str(listing_id), safe="-"),
                {"category_id": category_id}
            ...
            fixed.append({"code": "category_discovered", "value": category_id, "source": "mercado_livre"})
        else:
            unresolved.append({"code": "category_missing", "message": discovered.get("error")})

    if category_id:
        sync = await s21_sync_category(category_id)
        if not sync.get("success"):
            unresolved.append({"code": "category_metadata_unavailable", "message": "Não foi possível sincronizar os metadados da categoria."})
        else:
            removed = await s27_remove_non_official_attributes(product_id, category_id)
            if removed:
                fixed.append({"code": "non_official_attributes_removed", "attributes": removed})
            prepared = await s21_prepare(product_id, category_id)
            suggested = [
                row.get("attribute_id") for row in prepared.get("attributes") or []
                if row.get("status") in ("suggested", "filled") and row.get("value_name")
            ]
            missing_required = [
                row.get("attribute_id") for row in prepared.get("attributes") or []
            ...
            if suggested:
                fixed.append({"code": "category_attributes_prepared", "attributes": suggested})
            if missing_required:
                unresolved.append({"code": "required_attributes_missing", "attributes": missing_required})

    # Descrição: gera somente a partir dos dados já conhecidos no Product Master.
    if not str(product.get("description") or "").strip():
        description = s27_known_description(context)
        if description:
            await store.update(
                "products",
                "id=eq." + quote(str(product_id), safe="-"),
            ...
            fixed.append({"code": "description_built_from_known_data"})

    # Sincroniza preço e quantidade do anúncio com as fontes internas existentes.
    price = s19float(listing.get("price") or product.get("sale_price"), 0)
    quantity = s19int(listing.get("available_quantity") or (context.get("inventory") or {}).get("available"), 0)
    listing_patch = {}
    if s19float(listing.get("price"), 0) <= 0 and price > 0:
        listing_patch["price"] = price
        listing["price"] = price
        fixed.append({"code": "price_synced", "value": price})
    if listing.get("available_quantity") is None and quantity >= 0:
        listing_patch["available_quantity"] = quantity
        listing["available_quantity"] = quantity
        fixed.append({"code": "stock_synced", "value": quantity})
    if listing_patch:
        await store.update("listings", "id=eq." + quote(str(listing_id), safe="-"), listing_patch)

    # Recarrega e avalia os requisitos básicos antes de definir READY.
    context = await s19_product_context(product_id)
    local = s19_local_validation(context, listing)
    image_checks = [await s20_check_public_image(url) for url in context.get("images") or []]
    public_images_ok = bool(image_checks) and all(row.get("success") for row in image_checks)
    if not public_images_ok:
        unresolved.append({"code": "public_image_missing_or_invalid", "message": "Adicione uma imagem pública válida ao Product Master."})
    if not product.get("ean"):
        values = await s21_product_values(product_id, category_id) if category_id else {}
        empty_reason = values.get("EMPTY_GTIN_REASON") or values.get("empty_gtin_reason") or {}
        if not (empty_reason.get("value_id") or empty_reason.get("value_name")):
            unresolved.append({"code": "gtin_unresolved", "message": "Informe GTIN/EAN ou uma justificativa permitida pela categoria."})

    deterministic_ok = local.get("valid") and public_images_ok and not any(
        x.get("code") in {"required_attributes_missing", "category_missing", "category_metadata_unavailable", "gtin_unresolved"}
        for x in unresolved
    )
    target_status = "ready" if deterministic_ok else "draft"
    if str((context.get("product") or {}).get("internal_status") or "") != target_status:
        await store.update(
            "products",
            "id=eq." + quote(str(product_id), safe="-"),
            {"internal_status": target_status}
        )
        fixed.append({"code": "product_status_updated", "value": target_status})

    # Auditoria final: o Publishing Lab permanece como autoridade de liberação.
    listing = await s19_get_listing(listing_id)
    context = await s19_product_context(product_id)
    simulation = await s26_simulate(context, listing)
    run = await s26_store_run(listing, simulation)
    ...
    return {
        ...
        "publishing_lab": simulation,
        "run": run,
        "ready_to_publish": bool(simulation.get("success")),
    }


@app.post("/api/auto-correction/listing/{listing_id}")
async def auto_correction_listing(listing_id: str):
    result = await s27_auto_correct_listing(listing_id)
    if not result.get("success"):
        return JSONResponse(status_code=int(result.get("status_code") or 400), content=result)
    return result


@app.post("/auto-correction/listing/{listing_id}")
async def auto_correction_listing_page(listing_id: str):
    result = await s27_auto_correct_listing(listing_id)
    if not result.get("success"):
        return JSONResponse(status_code=int(result.get("status_code") or 400), content=result)
...
async def s29_process_product(product_id):
    ...
    if not enrichment.get("success"):
        await s19_history(
            listing_id, product_id, "workflow_enrichment_failed",
            "Enrichment Resolver não concluiu a busca automática.",
            None, listing.get("status") or "draft",
