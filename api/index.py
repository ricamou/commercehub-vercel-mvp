...
app = FastAPI(title="CommerceHub v2 Stable", version="v2-stable")
...
def calculate_price(cost_price, margin_percent=None, commission_percent=None, fixed_cost=None):
    ...
    if profit <= 0:
        ...
        recommendation = "critical_margin"
    elif real_margin < 18:
...
async def db_select(table):
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        response = await client.get(f"{SUPABASE_URL}/rest/v1/{table}?select=*", headers=await supabase_headers())
...
async def db_insert(table, payload):
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        response = await client.post(f"{SUPABASE_URL}/rest/v1/{table}", headers=await supabase_headers(), json=payload)
...
def optimize_listing(product):
    ...
    return {
        ...
        "seo_score": {
            ...
            "percentage": round(sum(1 for ok in checks.values() if ok) / len(checks) * 100, 2),
            "checks": checks
        },
        "warnings": warnings
    }
...
def parse_payload_by_type(source_type, payload):
    ...
    return {
        ...
        "count": len(products),
        "products": products,
        "validation": [validate_product(p) for p in products]
    }
    ...
    for item in supplier_items:
        ...
        if float(item.get("cost_price") or 0) != float(current.get("cost_price") or 0):
            ...
            cost_changes.append({
                "sku": sku,
                "name": item.get("name"),
                "old_cost": current.get("cost_price"),
                "new_cost": item.get("cost_price"),
                "suggested_sale_price": recalculated.get("sale_price"),
                "action": "update_cost_and_price"
            })
            changed = True
...
<body>
<aside>
<div class="logo"><b>CH</b><div><strong>CommerceHub</strong><span>v2 Stable</span></div></div>
<nav>
...
def table_rows(items, columns):
    rows = ""
    for item in items:
        rows += "<tr>" + "".join(f"<td>{item.get(col, '')}</td>" for col in columns) + "</tr>"
    return rows
...
<p><b>Versão:</b> CommerceHub v2 Stable</p></section>
"""
...
def enterprise_final():
    ...
    body = f"""
<section class="grid">
<div class="card"><span>Produtos</span><strong>{kpis['products']}</strong></div>
<div class="card"><span>Fornecedores</span><strong>{kpis['suppliers']}</strong></div>
<div class="card"><span>Estoque</span><strong>{kpis['stock']}</strong></div>
<div class="card"><span>Lucro potencial</span><strong>R$ {kpis['profit']:,.2f}</strong></div>
</section>
<section class="panel"><h2>Enterprise v1 Final — Stable</h2>
<p>Base consolidada sem imports externos entre módulos, pronta para Vercel.</p></section>
"""
...
<p>Conexão operacional por API, JSON, XML e CSV.</p>
<pre>GET /api/connectors/status
POST /api/connectors/parse/json
POST /api/connectors/parse/csv
POST /api/connectors/parse/xml</pre>
</section>
"""
...
<p>Importação, validação, separação de válidos/inválidos e persistência Supabase-ready.</p>
<pre>GET /api/import/summary/mock
POST /api/import/persist/json</pre>
</section>
"""
...
def sprint3():
    body = """
<section class="panel">
<h2>Sprint 3 — Supplier Sync Automation</h2>
<p>Comparação fornecedor x catálogo, custo, estoque, reprecificação e payload de marketplace.</p>
<pre>GET /api/sync/compare
GET /api/sync/marketplace-payload
POST /api/sync/run-demo</pre>
</section>
"""
    return layout("Sprint 3", body)
...
def fornecedores():
    rows = table_rows(SUPPLIERS, ["id", "name", "type", "status"])
...
def produtos():
    ...
    rows = ""
    for p in products:
        rows += f"<tr><td>{p['sku']}</td><td>{p['name']}</td><td>{p['brand']}</td><td>{stock_status(p['stock'])}</td><td>R$ {p['sale_price']:.2f}</td></tr>"
...
def anuncios():
    ...
    for p in all_products():
        rows += f"<tr><td>{p['sku']}</td><td>{p['name']}</td><td>{readiness(p)['ready']}</td><td><code>/api/anuncios/preview/{p['sku']}?category_id=MLBXXXX</code></td></tr>"
...
def relatorios():
    rows = ""
    for p in all_products():
        rows += f"<tr><td>{p['sku']}</td><td>{p['name']}</td><td>R$ {p['profit']:.2f}</td><td>{p['margin_percent']}%</td><td>{p['status']}</td></tr>"
...
def ai_page():
    ...
    for p in all_products():
        ...
        rows += f"<tr><td>{p['sku']}</td><td>{p['name']}</td><td>{optimized['title']}</td><td>{optimized['seo_score']['percentage']}%</td></tr>"
...
@app.get("/api/health")
def health():
    return {"status": "ok", "service": "commercehub", "version": "v2-stable"}
...
def api_connectors_status():
    return {
        "success": True,
        "module": "Universal Supplier Connector",
        "version": "stable",
        "connectors": [
            {"type": "api_rest", "name": "API REST", "status": "ready"},
            {"type": "json", "name": "JSON", "status": "ready"},
            {"type": "xml", "name": "XML", "status": "ready"},
            {"type": "csv", "name": "CSV", "status": "ready"},
        ]
    }
...
def api_connectors_parse(source_type: str, payload: dict):
    raw_payload = payload.get("payload", payload)
    return parse_payload_by_type(source_type, raw_payload)


@app.post("/api/import/from-payload/{source_type}")
def api_import_from_payload(source_type: str, payload: dict):
    raw_payload = payload.get("payload", payload)
    return import_from_payload(source_type, raw_payload)
...
def api_pricing_report(margin_percent: float = None):
    products = all_products()
    return {"success": True, "products": [{"sku": p["sku"], "name": p["name"], "suggested": calculate_price(p["cost_price"], margin_percent=margin_percent)} for p in products]}
...
def api_order_simulate():
    order = {
        ...
        "status": "paid",
        "total_amount": 89.90,
        "payload": {"items": [{"sku": "SUP-001", "quantity": 1, "price": 89.90}]},
        "created_at": datetime.utcnow().isoformat()
    }
...
def api_finance():
    ...
    return {
        ...
        "profit_potential": round(sum(p["profit"] * p["stock"] for p in products), 2),
        "products": products
    }
